...
(function($) {
  ...
  var has3d,
    ...
    turnMethods = {
      ...
      init: function(opts) {
        ...
        if (opts.direction !== "") this.turn("direction", opts.direction);
        ...
        $(this)
          .bind(mouseEvents.down, data.docEvents.mouseStart)
        ...
        $(document)
          .bind(mouseEvents.move, data.docEvents.mouseMove)
      ...
      destroy: function() {
        ...
        $(document)
          .unbind(mouseEvents.move, data.docEvents.mouseMove)
      ...
      _necessPage: function(page) {
        ...
        return (
          data.pageObjs[page].hasClass("fixed") ||
      ...
      removePage: function(page) {
        ...
        else turnMethods._makeRange.call(this);
        ...
        if (data.pageMv.length && data.pageMv[0] !== 0) {
          ...
          for (page in data.pageWrap) {
            ...
            if ((p = data.pages[page])) {
              ...
              if (data.tpage) p.flip("disable", true); // data.disabled || page!=apage
            }
          }
        } else {
          // Update static pages
    ...
    flipMethods = {
      ...
      init: function(opts) {
        this.data({
          f: {
            effect:
              opts.corners == "r" || opts.corners == "l" ? "hard" : "sheet"
          }
        });
      ...
      _cornerActivated: function(e) {
        var data = this.data().f,
          ...
          height = this.height(),
          c = {
            x: Math.max(0, e.pageX - pos.left),
            y: Math.max(0, e.pageY - pos.top)
          },
        ...
        if (c.x > width - csz) c.corner = "r";
        else if (c.x < csz) c.corner = "l";
        else return false;
      ...
      _moveFoldingPage: function(bool) {
        ...
        if (bool) {
          ...
          if (folding) {
            if (data.folding) {
              if (data.folding === folding) return;
  ...
  function decorator(that, methods, args) {
    ...
    else throw turnError(args[0] + " is an invalid value");
  }
