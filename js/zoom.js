  ...
  var has3d,
    ...
    zoomMethods = {
      ...
      zoomIn: function(event) {
        ...
        if (realPos.y > maxMove.y) move.y = realPos.y - maxMove.y + move.y;
        else if (realPos.y < minMove.y) move.y = realPos.y - minMove.y + move.y;

        if (realPos.x > maxMove.x) move.x = realPos.x - maxMove.x + move.x;
        else if (realPos.x < minMove.x) move.x = realPos.x - minMove.x + move.x;
      ...
      zoomOut: function(duration) {
        ...
        duration =
          typeof duration != "undefined" ? duration : data.opts.duration;
      ...
      scroll: function(to, unlimited, animate) {
        ...
        if (has3d) {
          ...
          flip.css(css);
          flip.transform(
            translate(-data.axis.x - to.x, -data.axis.y - to.y, true)
          );
        } else {
      ...
      _eMouseMove: function(event) {
        ...
        if (data.draggingCur) {
          ...
          $(this).zoom(
            "scroll",
            point2D(data.scrollPos.x - motion.x, data.scrollPos.y - motion.y),
            true
          );
      ...
      _eDestroying: function() {
        ...
        for (var eventName in data.flipbookEvents) {
          if (
            Object.prototype.hasOwnProperty.call(data.flipbookEvents, eventName)
          ) {
